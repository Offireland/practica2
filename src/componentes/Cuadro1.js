import "./Cuadro1.css";
export default function Cuadro1(){
    return(
    <>
   <div className="cj1"></div>
        <div className="c01"></div>
        <div className="c02"></div>
        <div className="c03"></div>
        <div className="c04"></div>

        <div className="c4"></div>
        <div className="c3"></div>
        <div className="c2"></div>
        <div className="c1"></div>

        <div className="c41"></div>
        <div className="c31"></div>
        <div className="c21"></div>
        <div className="c11"></div>

        <div className="c42"></div>
        <div className="c32"></div>
        <div className="c22"></div>
        <div className="c12"></div>

        <div className="c43"></div>
        <div className="c33"></div>
        <div className="c23"></div>
        <div className="c13"></div>
        <div className="cm"></div>
        <div className="cl1"></div>
        <div className="cl2"></div>
        <div className="cmn2"></div>
        <div className="cmn"></div>
    </>);
}
